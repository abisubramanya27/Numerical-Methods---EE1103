...
int main() {
   ...
   //Sum or squares of error for lagrange interpolation

   //Calculating sq_sum for every data point except those for which graph is drawn(every 50th point)

   //Not taking points beyond the range where we used interpolation

   //At the end the ealier version of the code pieces which are changed here to reduce the error, are appended

   for (int i = 0; i < (N /50) * 50; i++) {
   ...
   //We wont take those points (every fifth point) as it is with those points the interpolation is obtained and such terms give no error
   //Hence checking i % 50 != 0 to reduce running time of program
   ...
   //Sum or squares of error for newton interpolation
   ...
   //Sum or squares of error for cubic spline interpolation

   //We are only taking i from 0 to N - 2 , as 501 is the last point in the set of points we took to construct the cubic spline
...
//Lagrange function takes every 50th point to construct the function approximation to reduce error
...
//Newton function takes every 50th point to construct the function approximation to reduce error
...
double newton(double *y_data,int val_x,int N) {
   ...
   double val_y = 0, *dr = malloc(sizeof(double) * (N/50 + 1));
   ...
   for (int i = 0; i < N; i += 50) {
      ...
      //coeff gives each unknown coefficient of the newton polynomial
...
//Appending the code pieced changed from that in the earlier versions in lagrange and newton methods to reduce the errors those polynomials predict

//Finding error for lagrange and newton polynomial
